...
export default function Timeline() {
  return (
    <div className="relative flex flex-col items-center mx-8">
      <div className="font-WorkSans text-xl absolute left-4 top-16 bottom-20 h-[470px] lg:h-[540px] w-[3px] bg-orange-500 md:left-1/2 md:-translate-x-1/2" />

      {timelineData.map((item, index) => (
        <motion.div
          key={index}
          className="relative flex flex-col md:flex-row items-start md:items-center justify-between w-full max-w-3xl py-4 lg:py-8"
        ...
        >
          ...
          {index % 2 === 0 ? (
            ...
            <div className="hidden md:block w-1/2 text-right pr-6 font-medium text-xl text-gray-600">
              {item.title}
            </div>
          )}

          <div className="hidden md:flex relative items-center justify-center mx-10">
            <div className="bg-white p-[6px] rounded-full z-10">
              <div
                className={`bg-orange-500 ${
                  item.type === "start" || item.type === "end"
          ...
          {index % 2 === 0 ? (
            <div className="hidden md:block w-1/2 text-left pl-6 font-medium text-xl text-gray-600">
              {item.title}
            </div>
          ) : (
